...
def potential(phi):
    return (phi_c**6 - 7*a*(np.pi*b)**6)/phi_c**7 * phi**2 - 35*a*(np.pi * b)**4/(2 * phi_c**7) * phi**4
...
def soft_wall(phi, phiL, phiR):
    width = 0.1 * (phiR - phiL)        # how thick the boundary region is
    V0 = 1
    # zero in the interior, grows near edges
    left = np.clip((phiL + width - phi)/width, 0, None)
    right = np.clip((phi - (phiR - width))/width, 0, None)
    return V0 * (left**6 + right**6)
...
for b in b_values:
    ...
    temp1 = []
        # --- GRID ---
    phi_max = np.sqrt((phi_c**6 - 7*a*(np.pi*b)**6)/phi_c**7/ (2 * 35*a*(np.pi * b)**4/(2 * phi_c**7)))
    phi_min = -np.sqrt((phi_c**6 - 7*a*(np.pi*b)**6)/phi_c**7/ (2 * 35*a*(np.pi * b)**4/(2 * phi_c**7)))
    # phi_min, phi_max = -np.sqrt((phi_c**2)/3 - np.pi**2*b**2),np.sqrt((phi_c**2)/3 - np.pi**2*b**2)


    # phi_min, phi_max = -700,700
    N = 2400
    phi = np.linspace(-5, 5, N)
    dphi = phi[1] - phi[0]
    U = (phi_c**6 - 7*a*(np.pi*b)**6)/phi_c**7 * phi**2 - 35*a*(np.pi * b)**4/(2 * phi_c**7) * phi**4
    ...
    plt.plot(phi_range, U_function(phi_range) - np.min(U), linewidth=5)
    ...
    plt.grid(alpha=0.25)
    plt.tight_layout()
    plt.savefig(f"imgs\\7_energies_b{np.round(b,2)}.png")
    plt.show()
    plt.close()